      ...
      EditText newpasswordET = view.findViewById(R.id.newpasswordET);
        ...
        updatePasswordBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String newPossword = newpasswordET.getText().toString().trim();
                if(TextUtils.isEmpty(oldPassword)){
                    passwordET.setError("Password is required");
                    return;
                }
                if(TextUtils.isEmpty(newPossword)){
                    newpasswordET.setError("New Password is required");
                    return;
                }
                if(newPossword.length() < 6){
                    newpasswordET.setError("min length 6 characters");
                    return;
                }
                dialog.dismiss();
                updatePassword(oldPassword, newPossword);
            }
        });
    }

    private void updatePassword(String oldPassword, String newPossword) {
      pd.show();
      //get current user
        FirebaseUser user = firebaseAuth.getCurrentUser();
        //before change the password re-authenticate the user
        AuthCredential authCredential = EmailAuthProvider.getCredential(user.getEmail(), oldPassword);
        user.reauthenticate(authCredential)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
                       user.updatePassword(newPossword)
                               .addOnSuccessListener(new OnSuccessListener<Void>() {
                                   @Override
                                   public void onSuccess(Void aVoid) {
                                       pd.dismiss();
